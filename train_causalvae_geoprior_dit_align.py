...
def train(args):
    ...
    if args.align_weight > 0:
        ...
        if getattr(args, 'use_lora', False):
            ...
            # Cast LoRA params to fp32 — GradScaler does not support BFloat16 grads.
            _n_lora_params = 0
    ...
    if _has_trainable_vae:
        ...
        class _NoDDPWrapper:
            ...
            @property
            def training(self): return self.module.training
    ...
    if patchify_module is not None:
    ...
    # [NEW] LoRA params on student DiT (trainable). Same lr as args.lr.
        ...
        if lora_params:
            ...
            if global_rank == 0:
                logger.info(f"[LoRA] optimizer received {len(lora_params)} param tensors "
                            f"({sum(p.numel() for p in lora_params):,} elements) at lr={args.lr}")
